# Netflix Sync Chrome 擴展

基於 `netflix-sync-client.js` 邏輯改寫的 Chrome 擴展版本，可以讓多人同步觀看 Netflix 影片。

## 功能特色

- 🎬 同步播放/暫停狀態
- ⏰ 同步播放時間跳轉
- 👥 多人同時觀看
- 🏠 房間管理（建立、加入、離開）
- 📋 房間列表查看
- 🎯 簡單易用的介面
- 🔄 自動重連機制
- 🛠️ 手動控制功能

## 安裝步驟

### 1. 下載擴展文件

確保你有以下文件：
- `manifest.json`
- `popup.html`
- `popup.js`
- `background.js`
- `content.js`
- `icons/` 文件夾（包含圖標）

### 2. 修改伺服器地址（可選）

在 `content.js` 文件中，將 `CONFIG.SERVER_URL` 修改為你的伺服器地址：

```javascript
const CONFIG = {
  SERVER_URL: 'https://web-production-14c5.up.railway.app', // 預設伺服器
  // ...
};
```

同時更新 `manifest.json` 中的 `host_permissions`：
```json
"host_permissions": [
  "https://*.netflix.com/*",
  "https://your-server-url.com/*"
]
```

### 3. 在 Chrome 中安裝

1. 打開 Chrome 瀏覽器
2. 在地址欄輸入 `chrome://extensions/`
3. 開啟右上角的「開發者模式」
4. 點擊「載入未封裝項目」
5. 選擇包含擴展文件的文件夾
6. 擴展應該會出現在擴展列表中

## 使用方法

### 1. 打開 Netflix

- 前往 [Netflix](https://netflix.com)
- 開始播放任何影片

### 2. 建立房間

1. 點擊 Chrome 工具欄中的 Netflix Sync 圖標
2. 在彈出視窗中輸入房間 ID 和房間名稱
3. 點擊「建立房間」
4. 將房間 ID 分享給朋友

### 3. 加入房間

1. 在彈出視窗中輸入房間 ID
2. 點擊「加入房間」
3. 等待連接成功

### 4. 查看房間列表

1. 點擊「查看房間列表」按鈕
2. 在瀏覽器控制台查看所有可用房間

### 5. 同步觀看

- 當有人播放/暫停時，其他人的播放器會同步
- 當有人跳轉時間時，其他人也會跳轉到相同時間
- 可以在彈出視窗中看到連接狀態和當前房間

## 進階功能

### 手動控制

在瀏覽器控制台可以使用以下命令：

```javascript
// 手動播放
NetflixSync.play(timeInMilliseconds);

// 手動暫停
NetflixSync.pause();

// 手動跳轉
NetflixSync.seek(timeInMilliseconds);

// 獲取狀態
NetflixSync.getStatus();

// 獲取 Netflix API
NetflixSync.getNetflixAPI();
```

## 文件結構

```
chrome-extension/
├── manifest.json          # 擴展配置文件
├── popup.html            # 彈出視窗 HTML
├── popup.js              # 彈出視窗 JavaScript
├── background.js          # 背景腳本
├── content.js            # 內容腳本（核心邏輯）
├── README.md             # 主要說明文件
├── INSTALL.md            # 安裝指南
└── icons/                # 圖標文件夾
    ├── icon16.png
    ├── icon48.png
    └── icon128.png
```

## 技術細節

### 架構

- **Popup**: 用戶介面，處理用戶輸入和房間管理
- **Background**: 管理擴展生命週期
- **Content Script**: 注入到 Netflix 頁面，處理同步邏輯

### 通信機制

- Popup ↔ Content Script: 通過 `chrome.runtime.sendMessage`
- Content Script ↔ Netflix: 直接操作 Netflix API
- Content Script ↔ Server: Socket.IO WebSocket 連接

### Netflix API 使用

擴展使用 Netflix 的內部 API 來控制播放器：
- 獲取播放器實例
- 控制播放/暫停
- 監聽播放事件
- 跳轉播放時間
- 獲取當前播放時間

### 同步機制

1. **事件監聽**：監聽 Netflix 播放器的播放、暫停、跳轉事件
2. **Socket.IO 通信**：使用 WebSocket 與同步伺服器實時通信
3. **狀態同步**：將本地播放狀態廣播給房間內其他成員
4. **遠程控制**：接收其他成員的播放指令並執行

## 故障排除

### 常見問題

1. **無法找到播放器**
   - 確保正在播放 Netflix 影片
   - 重新載入頁面後再試

2. **無法連接伺服器**
   - 檢查 `CONFIG.SERVER_URL` 是否正確
   - 確保伺服器正在運行

3. **同步不工作**
   - 檢查瀏覽器控制台是否有錯誤
   - 確保所有用戶都在同一個房間
   - 查看連接狀態和房間信息

4. **房間操作失敗**
   - 確保房間 ID 格式正確
   - 檢查房間是否存在
   - 查看瀏覽器控制台錯誤信息

### 調試

1. 打開 Chrome 開發者工具
2. 在 Console 中查看 NetflixSync 的日誌
3. 檢查 Network 標籤中的 WebSocket 連接
4. 使用 `NetflixSync.getStatus()` 檢查狀態

## 開發

### 修改代碼

1. 修改文件後，在 `chrome://extensions/` 中點擊擴展的重新載入按鈕
2. 重新載入 Netflix 頁面

### 添加功能

- 在 `content.js` 中添加新的 API 方法
- 在 `popup.js` 中添加新的 UI 元素
- 在 `messageHandler` 中添加新的消息處理

### 本地開發

1. 修改 `content.js` 中的 `CONFIG.SERVER_URL` 為本地伺服器地址
2. 在 Chrome 擴展頁面點擊「重新載入」
3. 測試功能並查看控制台輸出

## 授權

此擴展基於原始 `netflix-sync-client.js` 腳本開發，僅供學習和個人使用。請遵守 Netflix 的使用條款。 